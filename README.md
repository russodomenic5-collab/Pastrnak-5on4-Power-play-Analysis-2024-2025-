...
* **The Solution:** We used **three files**—Team Level $\text{5v4 icetime}$ (total), $\text{Skater data (Pastrnak's icetime & rates}$, and $\text{shot data (Pastrnak's misses, shot on goal, & goal locations)\F}$ stats—to mathematically calculate the **uncontaminated OFF TOI denominator**. This ensures the $\text{OFF}$ rate is a true, accurate baseline.
