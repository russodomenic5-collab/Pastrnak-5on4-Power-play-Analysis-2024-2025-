...
* **The Solution:** We used **three files**—Team Level $\text{5v4 icetime}$ (total), Skater $\text{(Pastrnak's icetime and rates)}$, and Shot $\text{(Pastrnak's misses, shot on goal, and goal locations)}$ stats—to mathematically calculate the **uncontaminated OFF TOI denominator**. This ensures the $\text{OFF}$ rate is a true, accurate baseline.
